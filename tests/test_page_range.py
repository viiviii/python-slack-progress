import pytest
from book.page import Page, PageRange


# 범위 값은 `int type`이다
def test_range_values_type_is_int():
    _range = PageRange(0, 0)
...
def test_page_has_positive_values():
    page = Page(_all=PageRange(1, 1), goal=PageRange(1, 1))
...
def test_not_positive_to_raises():
    with pytest.raises(ValueError):
        Page(_all=PageRange(0, 1), goal=PageRange(1, 1))
    with pytest.raises(ValueError):
        Page(_all=PageRange(-1, 1), goal=PageRange(1, 1))


# 페이지가 3000보다 크면 예외가 발생한다
def test_exceeded_max_range_to_raises():
    with pytest.raises(ValueError):
        Page(_all=PageRange(1, 3000), goal=PageRange(1, 1))


# 시작 페이지는 마지막 페이지보다 작거나 같다
def test_first_page_is_not_greater_than_last_page():
    page = Page(_all=PageRange(1, 10), goal=PageRange(3, 3))
...
def test_first_page_is_greater_than_last_page_to_raises():
    with pytest.raises(ValueError):
        Page(_all=PageRange(100, 1), goal=PageRange(1, 1))
    with pytest.raises(ValueError):
        Page(_all=PageRange(1, 1), goal=PageRange(100, 1))
...
def test_all_page_not_contains_goal_page_to_raises():
    with pytest.raises(ValueError):
        Page(_all=PageRange(10, 100), goal=PageRange(10, 130))
    with pytest.raises(ValueError):
        Page(_all=PageRange(10, 100), goal=PageRange(3, 99))
